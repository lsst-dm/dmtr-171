...
from the DAQ to the L1 handoff machine for staff to be able to view the
images along with query the EFD about the image, and then transferred to
...
the base for comcam. ~ On the comcam DAQ network is the CC-header
service system, the CC-forwarder, CC-archiver, and on a base network the
...
and the LSP notebook environment. ~ This milestone includes many steps.
~ It will ensure that image data can be acquired from the camera, ~a
single YAML header file is generated for all 9 CCDs (or sensors),
ingested into butler in file systems, served through NFS for the base
systems, made available for DBB resources, sent to NCSA, ingested at
NCSA and made available for processing/viewing. ~ ~\\
...
gateway environments, GPFS file systems at NCSA, and the LSP at NCSA. ~~
  ...
  Images are being taken from the com-cam environment.~


  \subsection{Exit Criteria}
  Images are being ingested and available on the LSP for viewing and
processing by science staff at NCSA.~~
...
\item Additional Test Personnel involved:
  \begin{itemize}
    \item Test case \href{https://jira.lsstcorp.org/secure/Tests.jspa#/testCase/LVV-T1549}{LVV-T1549}: Michelle Gower and Michelle Butler~
...
recieve a image from the comcam DAQ and transfer the file to NCSA for
viewing on the LSP.~ ~
...
archiver/forwarder, AT-handoff, DM-butler, and DM butler/G3~
...
NCSA for viewing by LSP.~~


    \textbf{ Preconditions}:\\
    Comcam must be at the summit and working.~~
